...
REM =======================================================================
REM VoteBem Windows Development Environment Setup
REM
REM Purpose:
REM   - One-time setup script for a LOCAL development environment.
REM   - Checks for Python 3.11+.
REM   - Creates and activates a Python virtual environment (.venv).
REM   - Installs Python dependencies from requirements.txt.
REM   - Checks for Docker and attempts to start dev services (MariaDB/Redis)
REM     using docker-compose.dev-services.yml (if available).
REM   - Falls back to SQLite if Docker is missing or fails.
REM   - Runs Django migrations to initialize the database.
REM
REM Redundancy:
REM   - This is the primary setup script for developers working on the code
REM     locally (running Django on host, not in Docker).
REM   - Keep this script if you do local Python development.
REM   - It is NOT used for the production-like Docker deployment flow
REM     (deploy_docker_locally.bat handles that separately).
REM
REM Usage:
REM   - Run once when cloning the repo or when dependencies change.
REM =======================================================================
