...
REM =======================================================================
REM VoteBem Windows Local Development Startup (with Debugger)
REM
REM Purpose:
REM   - Starts the Django development server on the host (localhost:8000).
REM   - Activates the virtual environment created by setup.bat.
REM   - Configures local dev settings (DEBUG=True, SQLite fallback, console email).
REM   - Installs and configures `debugpy` to allow VS Code to attach to the
REM     running process for step-through debugging.
REM   - Does NOT run the app in Docker; it runs directly on Windows.
REM
REM Redundancy:
REM   - This is the primary startup script for ACTIVE DEVELOPMENT (coding/debugging).
REM   - Keep this script for daily coding tasks.
REM   - It is distinct from the production-like Docker flow (deploy_docker_locally.bat).
REM
REM Usage:
REM   - Run this to start coding.
REM   - Connect VS Code debugger using "Python: Remote Attach" to localhost:5678.
REM =======================================================================
